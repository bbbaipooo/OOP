...
public class TextEditor extends Application {
    ...
    public Label statusMessage = new Label("Checking for Changes..."); //Label is useful for displaying text that is required to fit within a specific space
    public ProgressBar progressBar = new ProgressBar(0.0);
    public Button loadChangesButton = new Button("Load Changes");

    public TextEditor(Stage primaryStage) {
        start(primaryStage);
    }
    ...
    private void initUI(Stage primaryStage) {
        ...
        //Font font = new Font("C:/javaProject/Project/OOP_PROJECT/src/FCPalette.otf", 20.0);
    ...
    public void saveFile() throws IOException {
        if (isNew) {
            try {
                FileWriter nFileWriter = new FileWriter(this.inputFileName);
                nFileWriter.write(textArea.getText());
                nFileWriter.close();
                System.out.println("Successfully wrote to the new file.");
            } catch (Exception e) {
                Logger.getLogger(getClass().getName()).log(SEVERE, null, e);
            }
...
//save fileธรรมดา ไปเก็บตรงไหน
//save fileแบบnew file มีการให้กรอกชื่อไฟล์ใหม่
//ทำnew file